...
export default function GuideBooking() {
  ...
  const fetchGuideInfo = async () => {
    try {
      const response = await axios.get('/api/guides/random');
      setGuide(response.data);
      setLoading(false);
    } catch (err) {
      setError('Failed to fetch guide information');
      setLoading(false);
    }
  };
  ...
  const handleSubmitRequirements = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('/api/guides/requirements', trekRequirements);
      setGuideFeedback(response.data.feedback);
    } catch (err) {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      await axios.post('/api/guides/book', bookingData);
        ...
        <Card className="md:col-span-1">
          <CardHeader>
            <Avatar className="w-24 h-24 mx-auto">
              <AvatarImage src={guide.avatarUrl} alt={`${guide.name}'s Avatar`} />
              <AvatarFallback>{guide.name.charAt(0)}</AvatarFallback>
            </Avatar>
            <CardTitle className="text-center mt-4">{guide.name}</CardTitle>
            <CardDescription className="text-center">{guide.expertise}</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <p className="mt-4 text-center">{guide.bio}</p>
          </CardContent>
        </Card>

        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle>Book Your Trek with {guide.name}</CardTitle>
            <CardDescription>Tell us about your ideal trek, and {guide.name} will provide personalized recommendations.</CardDescription>
          </CardHeader>
          <CardContent>
            {!guideFeedback ? (
              ...
              <div className="text-center space-y-4">
                ...
                <p>Thank you for booking your trek with {guide.name}. You will receive a confirmation email shortly with further details.</p>
              </div>
            ) : (
